"""
End-to-end pipeline: ingest → preprocess → features → train (baseline / gradient_boosting) → evaluate → save.

Run from project root:
  PYTHONPATH=. python -m price_estimation.src.pipeline
  or
  cd price_estimation && PYTHONPATH=.. python -m src.pipeline
"""
...
from .features.historical_price import build_historical_price_features, add_time_decay_weights
...
def _build_item_level_dataset(
    ...
    """
    Build one row per item_id: target = median sale price; features = historical + condition + genre/artist.
    Returns (X, y, item_ids, encoder_state).
    """
    ...
    exclude = {"item_id", "target_price", "sleeve_condition", "media_condition", "genre", "artist", "release_year", "year_col"}
...
def _time_based_split(
    ...
    """
    Assign each item to train/val/test by its latest sale date (time-based).
    Returns (train_item_ids, val_item_ids, test_item_ids).
    """
...
def run_pipeline(
    ...
    if not skip_ingest:
        ...
        if not sales_path.exists():
            ...
            metadata = pd.read_csv(processed_path / "metadata.csv") if (processed_path / "metadata.csv").exists() else pd.DataFrame()

    if sales.empty or "sale_price" not in sales.columns:
        raise ValueError(
            "No sales data. Add data/raw/sales.csv with columns: item_id, sale_price, sale_date (optional: sleeve_condition, media_condition)."
        )
    ...
    if X.empty or len(y) < 10:
        raise ValueError("Not enough items after feature building. Need at least 10 items with sufficient history.")
    ...
    if phase == "baseline":
        ...
        y_pred = point
        metrics = compute_metrics(y_test.values if hasattr(y_test, "values") else y_test, y_pred)
        target_cov = config.get("evaluation", {}).get("prediction_interval_coverage", 0.8)
    ...
    if log_mlflow:
        try:
            ...
            mlflow.set_experiment(mlflow_cfg.get("experiment_name", "price_estimation"))
