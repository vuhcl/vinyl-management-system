"""Serving-time PS ladder guardrails (trim + log-blend anchor nudge)."""
...
def reference_floor_usd(
    ...
    *,
    ...
    """max(median, average, credible lowest listing)."""
...
def is_inflated_ladder(
    ...
    *,
    ...
    if mx > ref * float(cfg.inflated_max_rung_to_reference):
        return True
    if mx > ref * float(cfg.inflated_max_rung_to_sale_reference):
        return True
...
def guardrails_active(
    ...
    *,
    ...
    if not cfg.enabled:
        return False
    return reference_floor_usd(stats, cfg, nm_grade_key=nm_grade_key) is not None
...
def sale_stats_blend_apply(
    ...
    *,
    ...
    if not guardrails_active(stats, cfg, nm_grade_key=nm_grade_key):
        return False
    if not is_inflated_ladder(stats, cfg, nm_grade_key=nm_grade_key):
        return False
    ref = reference_floor_usd(stats, cfg, nm_grade_key=nm_grade_key)
    mx = max_price_suggestion_ladder_usd(stats)
    if ref is None or mx is None or ref <= 0:
        return False
    if mx / ref > float(cfg.max_ladder_to_reference):
        return False
    return True
...
def trim_price_suggestions_json(
    ...
    *,
    ...
    for grade, entry in ladder.items():
        ...
        if mint_cap is not None and "mint" in g_lower and "near mint" not in g_lower and "(nm" not in g_lower:
            if val > mint_cap:
                new_val = float(nm * 1.15)
...
def prepare_stats_for_inference(
    stats: dict[str, Any],
    cfg: AnchorGuardrailsConfig,
    *,
    nm_grade_key: str,
...
def blend_path_anchor_usd(
    ...
    *,
    ...
    if rung <= 0 or not math.isfinite(rung):
        return rung
    if not sale_stats_blend_apply(stats, cfg, nm_grade_key=nm_grade_key):
        if warnings is not None and cfg.enabled and guardrails_active(
            stats, cfg, nm_grade_key=nm_grade_key
        ):
            if is_inflated_ladder(stats, cfg, nm_grade_key=nm_grade_key):
                ref = reference_floor_usd(stats, cfg, nm_grade_key=nm_grade_key)
                mx = max_price_suggestion_ladder_usd(stats)
                if ref and mx and mx / ref > float(cfg.max_ladder_to_reference):
                    if "sale_stats_blend_skipped" not in warnings:
                        warnings.append("sale_stats_blend_skipped")
        return rung
    ref = reference_floor_usd(stats, cfg, nm_grade_key=nm_grade_key)
    if ref is None:
        return rung
