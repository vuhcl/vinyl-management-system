...
def build_placeholder_grade_delta_fit(
    *,
    ...
    """Default / bootstrap artifact until a real cross-grade fit is wired."""
    return {
        "schema_version": 1,
        "fit_metadata": {
...
def write_grade_delta_scale_json(path: Path | str, blob: dict[str, Any]) -> None:
...
def validate_grade_delta_scale_fit_json(blob: Any) -> None:
    ...
    if not isinstance(blob, dict) or not blob:
        raise ValueError("grade_delta_scale fit JSON must be a non-empty object")
