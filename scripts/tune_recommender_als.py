...
def main() -> int:
    ...
    parser.add_argument(
        "--two-stage",
        action="store_true",
        help="Metadata candidate pool + ALS on pool (needs albums.parquet)",
    )
    parser.add_argument(
        "--max-candidates",
        type=int,
        default=2000,
        help="Max candidates per user when --two-stage",
    )
    ...
    print(
        f"sample rows: {effective_sample_n} users: {sample['user_id'].nunique()}"
    )
    print(f"grid size (capped): {len(grid)}")
    if args.two_stage:
        print("mode: two-stage retrieval + ALS on candidate pool")
