...
def _default_promo_phrases_from_file() -> tuple[str, ...]:
    """Load bundled default promo substrings (longest-first sort happens in :func:`load_promo_noise_patterns`)."""
    p = Path(__file__).resolve().parents[3] / "configs" / "default_promo_noise_phrases.json"
    with p.open(encoding="utf-8") as f:
        raw = json.load(f)
    if not isinstance(raw, list):
        raise TypeError(f"Expected JSON array of strings at {p}")
...
def load_promo_noise_patterns(pp_cfg: dict[str, Any]) -> tuple[str, ...]:
    ...
    if not raw:
        seq: Sequence[str] = _default_promo_phrases_from_file()
    else:
        seq = raw
